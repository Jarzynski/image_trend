...
Optimized training script for Jiang, Kelly, and Xiu-style 2D CNNs
...
Notes
-----
This script keeps the original CNN architecture unchanged. Most changes are
engineering-side throughput and memory optimizations.
"""
...
def fit_one_experiment(
    ...
    """
    Train one experiment using already-loaded window image shards and metadata.
    """
    ...
    log_path = TABLE_DIR / f"cnn_training_log_{output_stem(exp_name, options.arch)}.csv"

    print(
        f"Training {exp_name} on {device} | "
        ...
        f"spatial_dropout={options.spatial_dropout}"
    )
    ...
    model_path = MODEL_DIR / f"jiang_cnn2d_{stem}.pt"
    ...
    pred["model_name"] = model_name_for_arch(options.arch)
    pred["pred_prob"] = test_prob.astype(np.float32, copy=False)

    out_path = PRED_DIR / f"pred_{stem}_jiang_cnn2d.parquet"
